...
X_intercept_lst <- vector(mode = "list", length = K)
for (k in 1:K) {
  X_intercept_lst[[k]] <- cbind(rep(1, n_vec[k]), X_lst[[k]])
}


lambda1_cand <- seq(0.05, 0.5, 0.05) 
lambda2_cand <- seq(0.2, 0.8, 0.1) 

run.time <- system.time(optimal_tuning <- tuning_selection_logistic_cpp(theta_ini = debiased_beta, X_intercept_lst, Y_lst, K, p, 
                                                            N, n_vec, lambda1_cand, lambda2_cand, con = 1)) 
...
system.time(result_ip <- ICR_pg_logistic_cpp(theta_ini = debiased_beta, X_intercept_mat, Y_vec, n_vec, index,
                                             nu = 1, tau = 3, lambda1 = optimal_tuning[1], lambda2 = optimal_tuning[2], 
                                             eps_outer = 1e-5, max_iter = 500)) 

print(result_ip$iterations)





write.csv(lambda1_cand, file = paste('Example5_K90_n400_p100_IP_lambda1_cand_', seed, '.csv', sep = ""))
write.csv(lambda2_cand, file = paste('Example5_K90_n400_p100_IP_lambda2_cand_', seed, '.csv', sep = ""))
write.csv(run.time[3], file = paste('Example5_K90_n400_p100_IP_run.time_', seed, '.csv', sep = ""))
write.csv(result_ip$theta, file = paste('Example5_K90_n400_p100_IP_theta_', seed, '.csv', sep = ""))
write.csv(result_ip$alpha, file = paste('Example5_K90_n400_p100_IP_alpha_', seed, '.csv', sep = ""))
write.csv(theta_true, file = paste('Example5_K90_n400_p100_IP_thetatrue_', seed, '.csv', sep = ""))
